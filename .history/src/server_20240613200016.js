...
const { User, Food } = require('./mongodb');  
...
app.get('/index', async (req, res) => {
  try {
    ...
    res.render('index', { foods, loggedInUsername });
  } catch (error) {
...
app.get('/feed', async (req, res) => {
  try {
    const foods = await Food.find();
    const loggedInUsername = req.session.loggedInUsername;
    res.render('feed', { foods, loggedInUsername });
  } catch (error) {
    console.error('Error fetching foods:', error);
    res.status(500).send('Error fetching foods');
  }
...
app.post('/feed', upload.array('images', 10), async (req, res) => {
    try {
        const { title, description, deadline, price } = req.body;
        const feeder = req.session.loggedInUsername;
        const images = req.files.map(file => file.originalname); // Use file original names or handle as needed

        if (!feeder) {
            return res.status(400).send('User not logged in');
        }

        const foodAdded = new Food({
            title,
            description,
            deadline,
            price,
